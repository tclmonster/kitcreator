...
AC_DEFUN(DC_FIND_GOKIT_LIBS, [
	if test "${HAVE_GO}" = "yes"; then
		...
		for gopkgdir in ../../../*/inst/go-pkg/; do
			if test ! -d "${gopkgdir}"; then
				continue
			fi

			dnl Check that the directory contains at least one .go file
			gopkg_gofiles="`ls "${gopkgdir}"/*.go 2>/dev/null`"
			if test -z "${gopkg_gofiles}"; then
				continue
			fi

			gopkg_ext="`echo "${gopkgdir}" | sed 's@^\.\./\.\./\.\./@@;s@/inst/go-pkg/@@'`"
			gopkg_mod="kitcreator/ext/${gopkg_ext}"

			gokit_imports="${gokit_imports}	_ \"${gopkg_mod}\"
"
			gokit_requires="${gokit_requires}	${gopkg_mod} v0.0.0
"
...
"
			...
			if grep -q "${gopkg_initfunc}" ${gopkgdir}/*.go 2>/dev/null; then
				echo "extern int ${gopkg_initfunc}(Tcl_Interp *);" >> kitInit-libs.h
				libs_init_funcs="${libs_init_funcs} ${gopkg_initfunc}"
			fi
		done

		if test -n "${gokit_found}"; then
			AC_MSG_RESULT([${gokit_found}])
		else
			AC_MSG_RESULT([none])
		fi
